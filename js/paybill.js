...
paybill.addEventListener("click", async () => {
    ...
    if (!orderDeleted) {
        alert("⚠️ Không có order để thanh toán!");
        return;
    }
    ...
    await editById(URL_TABLE, updateTable);

    await deleted(URL_ORDER, orderDeleted.orderId);
        ...
        item.innerHTML = `
            <div class="card">
                ...
                <div class="card-footer">
                    ...
                    <p class="text-end" style="color: gray; font-style:italic;">
                        Payment made on ${s.createAt}
                    </p>
                </div>
            </div>`;
        ...
        s.bill.forEach(f => {
            ...
            pay_bill.innerHTML += `
                <li class="col d-flex align-items-center gap-2">
                    ...
                    <img class="img_pay" src="${food?.imgUrl}" alt="">
                    ${food?.name} x ${f?.quantity} = ${food?.price * f?.quantity} USD
                </li>`;
        });
